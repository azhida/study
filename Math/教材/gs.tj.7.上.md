...
例1 设 $ f:R \rightarrow R $ , 对每个 $ x \in R, f(x) = x^2 . $ 显然， $f$ 是一个映射，$f$ 的定义域 $D_f = R $, 值域 $ R_f = | y | y \geq 0 | $ ,它是 $R$ 的一个真子集。 对于 $ R_f $ 中的元素 $y$，除 $y = 0$ 外，它的原像不是唯一的，如 $y=4$ 的原像就有 $ x = 2 $ 和 $x = -2$ 两个.
