...
例2 设 $ X= \lbrace (x,y)|x^2+y^2=1 \rbrace , Y= \lbrace (x,0)| |x|<=1 \rbrace, f:X \rightarrow Y $ , 对每个 $(x,y) \in X$ , 有唯一确定的 $(x,0) \in Y$ 与之对应. 显然 $f$ 是一个映射，$f$ 的定义域 $D_f=X$ ，值域 $R_f=Y$ . 在几何上，这个映射表示将平面上一个圆心在原点的单位圆周上的点投影到 $x$ 轴的区间 $[-1,1]$ 上.
